<template>
    ...
    <div class="grid grid-cols-2 p-2 gap-2">
        <div @click="AddPop" class="cursor-pointer rounded-full font-bold bg-blue-500 px-4 w-full md:w-fit justify-center text-white text-xs p-2 flex items-center gap-2">
        ...
        <div class="rounded-full w-full bg-gray-200 px-4 text-white text-xs flex items-center gap-2">
            ...
            <input @keyup="handleSearch" v-model="search" type="text" class="p-2 w-full bg-gray-200 text-gray-800 focus:outline-none" placeholder="Search Negative Feedback"/>
        </div>
    </div>
        ...
        <table class="w-full table-auto">
            <thead>
                <tr class="bg-gray-100 text-xs">
                    ...
                    <th class="p-2 whitespace-nowrap border border-gray-200">Next Port</th>
            ...
            <tbody>
                <tr v-for="n in NF.data" @click="GoApproval(n.nf_id,n.ship_name,n.port_name,n.status_nf,n.approver)" class="text-xs text-center cursor-pointer">
                    ...
                    <td class="p-2 whitespace-nowrap border border-gray-200">{{ n.port_next_name }}</td>
                    ...
                    <td class="p-2 whitespace-nowrap border border-gray-200">
                        <div class="flex justify-center">
                            <div v-if="n.status_nf>=0" class="flex justify-center">
                                <FontAwesomeIcon :icon="n.status_nf ? faCheckCircle :faXmarkCircle" :class="n.status_nf ? 'text-green-500' : 'text-red-500'" size="2x"/>
                            </div>
                            <div v-if="n.status_nf==-1" class="rounded-full w-fit mx-auto px-4 py-1 bg-gray-600 text-white">
                                Need Approval
                            </div>
                        </div>
                    </td>
                    <td class="p-2 whitespace-nowrap border border-gray-200">
                        <div v-if="n.status_nf>=0" class="flex justify-center" @click="generateDoc($event,n.nf_id)"><FontAwesomeIcon :icon="faDownload" class="text-green-500" size="2x" /></div>
                    </td>
                </tr>
            </tbody>
        </table>
...
import { faCheckCircle, faDownload, faPlusCircle, faSearch, faXmarkCircle} from '@fortawesome/free-solid-svg-icons';
...
async function loadNF(){
    let res = await GetData('/list-negative-feedback',`search=${search}&limit=${PAGINATION.limit}&current_page=${(PAGINATION.current_page-1) * PAGINATION.limit}`)
...
const generateDoc = async (e,NF_ID) => {
  ...
  let nflist = {itemsnf:[]}
    NFDOWNLOAD.data.detail.map((a,i)=>{
        ...
        nflist.itemsnf.push({no:i+1,ref_number:a.ref_number,remarks:a.remarks})
    })
