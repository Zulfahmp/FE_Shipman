<script setup>
    ...
    const berth = reactive({berth_id:0,berth_name:'',berth_code:''})

    onMounted(()=>{
        berth.berth_id = data_berth.berth_id
        berth.berth_name = data_berth.berth_name
        berth.berth_code = data_berth.berth_code
    })
    ...
    async function Submit(){
        ...
        let res = await PostData('/edit-master-berth',berth)
...
<template>
    <div class="fixed h-screen z-10 top-0 left-0 right-0 flex items-center p-4 w-full" style="background-color: rgba(0,0,0,0.5);">
        <form @submit.prevent="Submit" class="bg-white rounded-lg w-full md:w-96 mx-auto">
            <div class="bg-blue-500 rounded-t-lg text-white p-2 font-bold text-center">Edit Master Data Berth</div>
