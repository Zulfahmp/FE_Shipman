<template>
    ...
    <div class="grid grid-cols-2 p-2 gap-2">
        <div @click="AddPop" class="cursor-pointer w-full md:w-fit rounded-full font-bold bg-blue-500 px-4 justify-center text-white text-xs p-2 flex items-center gap-2">
            <FontAwesomeIcon :icon="faPlusCircle" class="pointer-events-none"/>
            <div class="pointer-events-none whitespace-nowrap">New SSCL</div>
        </div>
        <div class="rounded-full w-full bg-gray-200 px-4 text-white text-xs flex items-center gap-2">
            <FontAwesomeIcon :icon="faSearch" class="text-gray-400"/>
            <input @keyup="handleSearch" v-model="search" type="text" class="p-2 w-full bg-gray-200 text-gray-800 focus:outline-none" placeholder="Search Ship Shore Safety Checklist"/>
        </div>
    </div>
    <div class="text-xs flex gap-1 items-center ml-3 mb-2">
        <div>Rows</div>
        <select v-model="PAGINATION.limit" class="border rounded-sm p-1 outline-none">
            <option>25</option>
            <option>50</option>
            <option>100</option>
            <option>150</option>
    ...
    <div class="overflow-x-scroll scrollbar-thin w-full px-2">
        <table class="w-full table-auto">
            ...
            <tbody>
                <tr v-for="s in sscl.data" @click="Page('preview-sscl/'+s.sscl_id,routing)" class="cursor-pointer text-xs text-center">
                    ...
                    <td class="p-2 whitespace-nowrap border border-gray-200">
                        <div v-if="s.status_sscl>=0" class="flex justify-center">
                            <FontAwesomeIcon :icon="s.status_sscl ? faCheckCircle :faXmarkCircle" :class="s.status_sscl ? 'text-green-500' : 'text-red-500'" size="2x"/>
                        </div>
                        <div v-if="s.status_sscl==-1" class="rounded-full w-fit mx-auto px-4 py-1 bg-gray-600 text-white">
                            Need Approval
                        </div>
                    </td>
                    <td class="p-2 whitespace-nowrap border border-gray-200">
                        <div v-if="s.status_sscl!=-1" class="flex justify-center"><FontAwesomeIcon :icon="faDownload" class="text-green-500" size="2x" @click="generateDoc($event,s.sscl_id)"/></div>
                    </td>
                </tr>
                <tr v-if="!sscl.data.length">
                    <td colspan="8" class="text-center text-gray-800 py-2 text-sm border border-gray-200">There is no SSCL</td>
                </tr>
            </tbody>
        </table>
        
        <div class="py-2 flex flex-wrap gap-2 justify-center w-full mx-auto px-10 mt-5">
            <div v-for="page in PAGINATION.total_page<=20 ? PAGINATION.total_page : PAGINATION.range_end" :id="page" @click="Pagination(page)" :class="page==PAGINATION.current_page ? 'bg-blue-700' : 'bg-gray-700'" class="cursor-pointer hover:bg-blue-600 active:bg-blue-600 text-white py-1 px-2 text-xs w-fit rounded-sm">{{ page }}</div>
        </div>
...
import { faCheckCircle, faDownload, faPlusCircle, faSearch, faSquare, faXmarkCircle} from '@fortawesome/free-solid-svg-icons';
...
let PAGINATION = reactive({current_page:1,total_page:0,range_start:1,range_end:20,limit:25,step:3})
...
async function loadSSCL(){
    let res = await GetData('/list-sscl',`search=${search}&limit=${PAGINATION.limit}&current_page=${(PAGINATION.current_page-1) * PAGINATION.limit}`)
...
const generateDoc = async (e,SSCL_ID) => {
  ...
  let remarks = {}
    SSCLDOWNLOAD.data.sscl_checklist.map(a=>{
        ...
        remarks[`${a.part_id}_${a.order_number}_remarks`]= a.remarks!="" ? a.remarks : '';
    })
    // REPETITIVE.summary.reverse()
    let part8 = {}
    let part9 = {}
    let HRSpart = {'8_HRS':REPETITIVE.summary[0].interval_8,'9_HRS':REPETITIVE.summary[0].interval_9}
    let Tpart = {}
    let time6limit = 1
    let summary = 0
    for(const a of REPETITIVE.summary){
        ...
        for(const b of REPETITIVE.detail[summary]){
            if(a.sscl_id==b.id_sscl && b.part_id=="8"){
                part8['8_'+b.order_number+'_T'+time6limit] = b.checker && b.nahkoda ? '☑ Yes' : '☐ Yes' ;
                part8['8_'+b.order_number+'_remarks'] = b.remarks
            }
            if(a.sscl_id==b.id_sscl && b.part_id=="9"){
                part9['9_'+b.order_number+'_T'+time6limit] = b.checker && b.nahkoda ? '☑ Yes' : '☐ Yes' ;
